...
import { Sparkles, Mic, Unlock } from 'lucide-react';
...
export const PricingCard: React.FC<PricingCardProps> = ({
    ...
    // Render content based on layout (Desktop vs Mobile styles are slightly different in structure but theme is shared)
    if (isDesktop) {
        return (
            <div
                onClick={() => onEnter(id)}
            ...
            >
                ...
                <div className="relative z-10 flex flex-col h-full items-center">
                    <div className={`w-24 h-24 rounded-full border ${isSun ? 'border-yellow-500/40' : 'border-stone-600'} flex items-center justify-center bg-black/60 mb-8 ${theme.iconBg} group-hover:scale-110 transition-all duration-500 shadow-[0_0_30px_rgba(250,204,21,0.1)] ${isSun ? 'group-hover:shadow-[0_0_60px_rgba(250,204,21,0.6)]' : 'group-hover:shadow-[0_0_50px_rgba(255,255,255,0.6)]'} relative overflow-hidden`}>
                        <div className={`absolute inset-0 ${isSun ? 'bg-yellow-400/10' : 'bg-white/0 group-hover:bg-white/20'} blur-md animate-pulse`}></div>
                        <Icon className={`w-10 h-10 ${theme.iconColor} ${theme.iconHover} transition-colors relative z-10 ${isSun ? 'animate-[spin_8s_linear_infinite]' : 'animate-[pulse_4s_infinite]'}`} />
                    </div>

                    <h3 className={`text-3xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-b ${theme.gradientText} mb-2 ${theme.gradientHover} transition-all drop-shadow-lg`}>{title}</h3>
                    <p className={`text-[10px] uppercase tracking-[0.4em] ${theme.subTextColor} mb-6 transition-colors font-bold`}>{subtitle}</p>

                    <p className={`text-stone-400 text-sm leading-relaxed mb-6 flex-grow px-4 ${isSun ? 'group-hover:text-yellow-100/90' : 'group-hover:text-stone-300'} transition-colors`}>
                        {description}
                    </p>

                    <div className="mb-6 flex flex-col items-center">
                        <span className={`${isSun ? 'text-lux-gold' : 'text-stone-300'} text-2xl font-bold`}>{price}</span>
                        <span className="text-stone-500 text-xs line-through">{priceFull}</span>
                    </div>

                    <button className={`w-full py-4 border ${theme.buttonBorder} transition-all text-sm tracking-[0.15em] font-bold uppercase flex items-center justify-center gap-2 rounded-sm ${theme.buttonShadow}`}>
    ...
    return (
        <div
            onClick={() => onEnter(id)}
            className={`relative w-full max-w-[340px] mx-auto min-h-[520px] rounded-2xl overflow-hidden flex flex-col justify-center items-center text-center border ${theme.mobileBorder} bg-lux-navy/80 backdrop-blur-xl ${theme.mobileShadow}`}
        >
            {/* Background Effects */}
            ...
            <div className="relative z-10 flex flex-col h-full items-center justify-between py-8 px-4">
                {/* Icon */}
                <div className={`w-28 h-28 rounded-full border ${isSun ? 'border-yellow-500/80 bg-yellow-500/10' : 'border-stone-400/80 bg-white/5'} flex items-center justify-center mb-6 ${theme.mobileIconShadow} relative overflow-visible mt-4`}>
                    ...
                    <Icon className={`w-12 h-12 ${isSun ? 'text-yellow-300 animate-[spin_4s_linear_infinite]' : 'text-white animate-[pulse_2s_infinite]'} relative z-10`} />
                </div>

                <div className="flex flex-col items-center">
                    <h3 className={`text-3xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-b ${isSun ? 'from-yellow-50 to-yellow-500' : 'from-stone-100 to-stone-500'} mb-3 drop-shadow-xl leading-tight`}>{title}</h3>
                    <p className={`text-[10px] uppercase tracking-[0.4em] ${isSun ? 'text-yellow-500' : 'text-stone-300'} mb-8 font-bold border-b ${isSun ? 'border-yellow-500/30' : 'border-stone-500/50'} pb-2`}>{subtitle}</p>
                    <p className="text-stone-300 text-sm leading-relaxed mb-8 px-2 font-light">
                        {description}
                    </p>
                </div>

                <button className={`w-4/5 py-4 border ${isSun ? 'border-yellow-500/50 text-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20' : 'border-stone-600/50 text-stone-300 bg-white/10 hover:bg-white/20'} transition-all text-xs tracking-[0.25em] font-bold uppercase flex items-center justify-center gap-2 rounded-sm shadow-[0_0_20px_rgba(255,255,255,0.2)] mb-4`}>
