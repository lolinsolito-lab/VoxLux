...
import { Mic, Video, Image, Box, FileAudio, Keyboard, Aperture, Activity, LogOut, Hexagon, HelpCircle } from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { id: View.SUPPORT, label: 'Help Center', icon: <HelpCircle className="w-4 h-4" />, description: 'FAQ & Support' },
];
      ...
      <div className="flex-1 py-2 md:py-8 px-2 md:px-4 flex flex-row md:flex-col gap-1 md:space-y-1 overflow-x-auto md:overflow-visible items-center md:items-stretch">
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`min-w-[60px] md:w-full text-left px-3 md:px-4 py-2 md:py-3 transition-all duration-300 flex flex-col md:flex-row items-center md:gap-4 group relative border border-transparent rounded md:rounded-none ${currentView === item.id
